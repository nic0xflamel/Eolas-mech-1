...
def get_synth_predictions(clients: APIClients, asset: str) -> Optional[list]:
...
def get_price_prediction_chart(best_prediction_data: list, asset_predictions: list, asset: str) -> str:
    ...
    price_ax.plot(df.close, label=f'Median Price Prediction({asset})', color='blue')
    price_ax.set_title(f'{asset} Price Simulations')
...
def run(
    **kwargs: Any,
    ...
    try:
        ...
        asset = "BTC" # TODO: Have this as a user input

        asset_predictions = get_synth_predictions(clients=clients, asset=asset)

        if not asset_predictions:
            return f"Failed to get price predictions from Synth subnet.", "", None, None
