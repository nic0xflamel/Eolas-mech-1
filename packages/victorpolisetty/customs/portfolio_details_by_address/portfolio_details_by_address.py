...
def get_wallet_portfolio_request(**kwargs) -> Tuple[Optional[str], Optional[Dict[str, Any]], Any, Any]:
    ...
    url = "https://api.compasslabs.ai/v0/generic/portfolio/get"
    payload = {
        "chain": chain,
        "user": user
    }
...
def run(**kwargs) -> MechResponse:
    ...
    # Fetch portfolio
    error, portfolio_data, _, _ = get_wallet_portfolio_request(chain=chain, user=user)
    
    if error:
        return f"Failed to retrieve wallet portfolio: {error}", "", None, None
    
    # Generate prompt incorporating the portfolio data
    prompt = f"""
    You are a financial analyst providing insights into the portfolio of a specific wallet address.
    Here is the retrieved portfolio data:
    {json.dumps(portfolio_data, indent=4)}
    Provide an analysis explaining the total value, token distribution, and potential investment strategies.
    """
    ...
    try:
        response = clients.openai_client.chat.completions.create(
            model="gpt-4o",
            messages=[
                {"role": "system", "content": "You are a financial analyst providing insights into blockchain wallet portfolios."},
        ...
        metadata_dict = {
            "question": f"Wallet portfolio for {user}",
