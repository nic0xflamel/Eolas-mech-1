...
def run(**kwargs) -> Tuple[Optional[str], Optional[Dict[str, Any]], Any, Any]:
    ...
    Args:
        kwargs (dict): Dictionary containing configuration parameters and commands
            Required keys:
    ...
    try:
        ...
        if 'selection_params' in kwargs:
            analyzer.set_parameters(selection_params=kwargs['selection_params'])
        if 'grid_params' in kwargs:
            analyzer.set_parameters(grid_params=kwargs['grid_params'])
        ...
        if command == 'scan':
            ...
            if not symbols:
                return "Grid Trading Analyzer failed. Error returned", "", {'status': 'error', 'message': 'No symbols provided for analysis'}, None
            ...
            if not symbols:
                return "Grid Trading Analyzer failed. Error returned", "", {'status': 'error', 'message': 'No suitable pairs found for monitoring'}, None
                
            # Set up monitoring parameters
            interval = kwargs.get('monitor_interval', 300)  # 5 minutes default
            duration = kwargs.get('monitor_duration', 3600)  # 1 hour default
            ...
            if not monitor_id or not hasattr(run, 'monitoring_sessions') or monitor_id not in run.monitoring_sessions:
                return "Grid Trading Analyzer failed. Error returned", "", {'status': 'error', 'message': 'Invalid or expired monitoring session ID'}, None
            ...
            if kwargs.get('save_report'):
                filename = kwargs.get('report_filename', 'grid_trading_report.csv')
            ...
            if not symbols or len(symbols) == 0:
                return "Grid Trading Analyzer failed. Error returned", "", {'status': 'error', 'message': 'Symbol required for grid export'}, None
                
            symbol = symbols[0]  # Take the first symbol
            filename = kwargs.get('export_filename')
            
            config = analyzer.export_grid_config(symbol, filename)
            
            if not config:
                return "Grid Trading Analyzer failed. Error returned", "", {'status': 'error', 'message': f'No valid grid configuration for {symbol}'}, None
            ...
            if not symbols or len(symbols) == 0:
                return "Grid Trading Analyzer failed. Error returned", "", {'status': 'error', 'message': 'Symbol required for visualization'}, None
            ...
            if kwargs.get('save_visualization'):
                filename = kwargs.get('visualization_filename', f'{symbol.replace("/", "_")}_grid.png')
            ...
            output = {
                ...
                'saved': kwargs.get('save_visualization', False),
                'filename': kwargs.get('visualization_filename') if kwargs.get('save_visualization') else None
            }
            
        else:
            return {'status': 'error', 'message': f'Unknown command: {command}'}
            
        return "Grid Trading Analyzer successfully executed. Metadata returned", "", output, None
        
    except Exception as e:
        import traceback
        return "Grid Trading Analyzer failed. Error returned", "", {
            'status': 'error',
            'message': str(e),
